...
description: >-
  ...
  "这几篇是不是同一个人写的""这些能不能免费下载 / 有没有全文""哪几篇是开放获取（OA）的""有没有 PMC 全文"也应使用。中英双轨：英文接开放
  ...
  被引数与开放获取状态只陈列不排序、不当阅读优先级，只给源提供的合法开放版本链接与官方获取入口、
  绝不提供绕过付费墙的路径（不给 Sci-Hub 类镜像站、不代抓订阅内容、不批量代下）；同名作者归并只认 ORCID 这个客观键，绝不按姓名 / 机构猜同一人，也不替用户选是哪位候选；
