...
3. **中文文献绝不进 NOT_FOUND、绝不标编造嫌疑**。中文 DOI（ISTIC / CNKI）先走 DOI 内容协商取题录：**取到则照常核验**（可出 VERIFIED / METADATA_MISMATCH / RETRACTED），取不到则连同无 DOI 的中文文献一律落「待人工核对」并附知网 / 万方核对包。「英文库查不到」≠「编造」≠「不存在」——可能是中文库文献或元数据未收录。这是硬约束⑤铁律。

   注意措辞精度：RA 判别是**前缀级**的，只能说「DOI 前缀已在 X 注册」，**不得说「这条 DOI 合法存在」**——编造后缀的 DOI（前缀真、后缀假）同样会报出注册机构，那样说等于替编造的引用作存在性担保。
